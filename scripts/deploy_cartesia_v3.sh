#!/usr/bin/env bash
#
# deploy_cartesia_v3.sh
#
# Deploy the Marathi female F5-TTS model fine-tuned (4 epochs) on the COMPLETE
# combined dataset (1630 Rasa ~3h + 1033 Cartesia ~88min) WITH the expanded
# vocabulary (138 tokens) that fixes ॅ (CANDRA E) and other missing characters.
#
# New checkpoint:
#   /root/f5-tts-marathi/f5tts/lib/python3.12/ckpts/Cartesia_Rasa_Combined_v3/model_last.pt
# Vocabulary (expanded, MUST be the exact file the model was trained with):
...
#   2. Forces emotion_layer/synthesize.py to the v3 checkpoint + extended vocab.
#   3. Forces the model identifier in emotion_layer/api.py to v3.
#   4. Restarts FastAPI on port 8000 and Gradio UI on port 7861.
#   5. Validates health, model-info, and synthesis (incl. ॅ target words).
#   6. Prints rollback procedure.
#
# Prior checkpoints (Rasa_Marathi_Emotion_Female, Cartesia_Rasa_Combined_3h,
# Cartesia_Rasa_Combined_v2) are NOT deleted; they remain as rollback options.
#
# Usage:
#   bash scripts/deploy_cartesia_v3.sh
#
set -euo pipefail
...
NEW_CKPT="/root/f5-tts-marathi/f5tts/lib/python3.12/ckpts/Cartesia_Rasa_Combined_v3/model_last.pt"
...
echo "============================================================"
echo " F5-TTS Marathi Deployment - Cartesia_Rasa_Combined_v3"
echo "============================================================"
...
# ---------------------------------------------------------------------------
# 2 & 3. Force synthesize.py / api.py to v3 paths
# ---------------------------------------------------------------------------
echo ""
echo "[2] Updating checkpoint & vocabulary in synthesize.py..."

# Force checkpoint path to v3 regardless of what is hardcoded
# (Rasa_Marathi_Emotion_Female / Cartesia_Rasa_Combined_3h / Cartesia_Rasa_Combined_v2).
perl -pi -e 's{/root/f5-tts-marathi/f5tts/lib/python3\.12/ckpts/(Rasa_Marathi_Emotion_Female|Cartesia_Rasa_Combined_3h|Cartesia_Rasa_Combined_v2)/model_last\.pt}{/root/f5-tts-marathi/f5tts/lib/python3.12/ckpts/Cartesia_Rasa_Combined_v3/model_last.pt}g' "$SYNTH"
...
perl -pi -e 's{(Rasa_Marathi_Emotion_Female|Cartesia_Rasa_Combined_3h|Cartesia_Rasa_Combined_v2)/model_last\.pt}{Cartesia_Rasa_Combined_v3/model_last.pt}g' "$API"
...
grep -n "Cartesia_Rasa_Combined_v3\|model_identifier" "$API" | head -10 || true
...
nohup "$PYTHON" -m uvicorn emotion_layer.api:app --host 0.0.0.0 --port $API_PORT \
    > "$LOG_DIR/api_deploy_v3.log" 2>&1 &
disown
NEW_API_PID=$!
echo "  Started new API process PID $NEW_API_PID (log: $LOG_DIR/api_deploy_v3.log)"
...
nohup "$PYTHON" emotion_layer/ui.py --port $UI_PORT \
    > "$LOG_DIR/ui_deploy_v3.log" 2>&1 &
disown
NEW_UI_PID=$!
echo "  Started new UI process PID $NEW_UI_PID (log: $LOG_DIR/ui_deploy_v3.log)"
...
grep -E "Loading|checkpoint|vocab|Error|Traceback|error" "$LOG_DIR/api_deploy_v3.log" | tail -30 || true
...
# ---------------------------------------------------------------------------
# 8. Synthesis validation (emotion regression + ॅ target words)
# ---------------------------------------------------------------------------
echo ""
...
TEST_CASES=(
    ...
    "NEUTRAL|तिने हॅलो म्हणत फोन उचलला."
...
for tc in "${TEST_CASES[@]}"; do
    i=$((i+1))
    emotion="${tc%%|*}"; text="${tc#*|}"
    out_file="$REPO_ROOT/validation_output/test_v3_${i}.wav"
...
' 2>/dev/null)
    if [[ -z "$base64str" || "$base64str" == "__PARSE_ERR__" ]]; then
        ...
        echo "    log:  $(tail -3 "$LOG_DIR/api_deploy_v3.log")"
        continue
    fi
...
echo "      cd $REPO_ROOT && nohup $PYTHON -m uvicorn emotion_layer.api:app --host 0.0.0.0 --port $API_PORT > $LOG_DIR/api_rollback_v3.log 2>&1 & disown"
echo "      kill \$(ss -ltnp | grep ':$UI_PORT ' | grep -oP 'pid=\\K[0-9]+' | head -1); sleep 3"
echo "      cd $REPO_ROOT && nohup $PYTHON emotion_layer/ui.py --port $UI_PORT > $LOG_DIR/ui_rollback_v3.log 2>&1 & disown"
echo "============================================================"
echo ""
echo "IMPORTANT: Confirm the API startup log loaded Cartesia_Rasa_Combined_v3"
