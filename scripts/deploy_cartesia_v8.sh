#!/usr/bin/env bash
# =============================================================================
# deploy_cartesia_v8.sh
#
# Deploy the targeted ॅ/ॲ model (Chandra_Vyanjan_Focus_v8) to the live
# services: FastAPI on port 8000 and Gradio UI on port 7861.
#
# New checkpoint:
#   /root/f5-tts-marathi/f5tts/lib/python3.12/ckpts/Chandra_Vyanjan_Focus_v8/model_last.pt
# Vocabulary:
#   /root/f5-tts-marathi/f5tts/data/Rasa_Marathi_Emotion_Female/vocab_extended.txt
#   (138 tokens, or 139/140 if v8_step0 embedding surgery appended chars)
#
# Validation suits:
#   Tests  1- 4: the 4 original chandra-vyanjan example words
#                 (मॅन कॅन लॅन व्हॅन)
#   Tests  5- 9: 5 ADDITIONAL ॅ/ॲ words NOT among the 4 examples
#                 (generalization check, not memorization)
#   Tests 10-13: more generalization sentences containing ॅ/ॲ
#   Tests 14-19: emotion-layer regression (NEUTRAL/SAD/HAPPY/ANGRY/FEAR/DISGUST)
#
# Steps:
#   1. Backup production files (synthesize.py, api.py).
#   2. Force emotion_layer/synthesize.py to v8 checkpoint + extended vocab.
#   3. Force the model identifier in emotion_layer/api.py to v8.
#   4. Restart FastAPI (8000) + Gradio UI (7861).
#   5. Validate health, model-info, and synthesis.
#   6. Print rollback procedure.
#
# Usage:
#   bash scripts/deploy_cartesia_v8.sh
# =============================================================================
set -euo pipefail
...
NEW_CKPT="/root/f5-tts-marathi/f5tts/lib/python3.12/ckpts/Chandra_Vyanjan_Focus_v8/model_last.pt"
...
echo "============================================================"
echo " F5-TTS Marathi Deployment - Chandra_Vyanjan_Focus_v8 (ॅ/ॲ)"
echo "============================================================"
...
if found is None:
    ...
    print("        This is a stale/wrong checkpoint. Aborting.")
    sys.exit(1)
else:
...
perl -pi -e 's{(Rasa_Marathi_Emotion_Female|Cartesia_Rasa_Combined_3h|Cartesia_Rasa_Combined_v2|Cartesia_Rasa_Combined_v3|Cartesia_Rasa_Combined_v4|Cartesia_Rasa_Combined_v5|Cartesia_Rasa_Combined_v6|Cartesia_Rasa_Combined_v7)/model_last\.pt}{Chandra_Vyanjan_Focus_v8/model_last.pt}g' "$API"
...
grep -n "Chandra_Vyanjan_Focus_v8\|model_identifier" "$API" | head -10 || true
...
nohup "$PYTHON" -m uvicorn emotion_layer.api:app --host 0.0.0.0 --port $API_PORT \
    > "$LOG_DIR/api_deploy_v8.log" 2>&1 &
disown
NEW_API_PID=$!
echo "  Started new API process PID $NEW_API_PID (log: $LOG_DIR/api_deploy_v8.log)"
...
nohup "$PYTHON" emotion_layer/ui.py --port $UI_PORT \
    > "$LOG_DIR/ui_deploy_v8.log" 2>&1 &
disown
NEW_UI_PID=$!
echo "  Started new UI process PID $NEW_UI_PID (log: $LOG_DIR/ui_deploy_v8.log)"
...
grep -E "Loading|checkpoint|vocab|Error|Traceback|error" "$LOG_DIR/api_deploy_v8.log" | tail -30 || true
...
TEST_CASES=(
    ...
    "NEUTRAL|हे पॅकेज उद्या येईल."
    "NEUTRAL|कोणाचे कॅलेंडर येथे आहे?"
    "NEUTRAL|मी पॅनमध्ये सॅलड बनवले."
...
for tc in "${TEST_CASES[@]}"; do
    i=$((i+1))
    emotion="${tc%%|*}"; text="${tc#*|}"
    out_file="$REPO_ROOT/validation_output/test_v8_${i}.wav"
...
' 2>/dev/null)
    if [[ -z "$base64str" || "$base64str" == "__PARSE_ERR__" ]]; then
        ...
        echo "    log:  $(tail -3 "$LOG_DIR/api_deploy_v8.log")"
        continue
    fi
...
echo "      cd $REPO_ROOT && nohup $PYTHON -m uvicorn emotion_layer.api:app --host 0.0.0.0 --port $API_PORT > $LOG_DIR/api_rollback_v8.log 2>&1 & disown"
echo "      kill \$(ss -ltnp | grep ':$UI_PORT ' | grep -oP 'pid=\\K[0-9]+' | head -1); sleep 3"
echo "      cd $REPO_ROOT && nohup $PYTHON emotion_layer/ui.py --port $UI_PORT > $LOG_DIR/ui_rollback_v8.log 2>&1 & disown"
echo "============================================================"
echo ""
echo "IMPORTANT: Confirm the API startup log loaded Chandra_Vyanjan_Focus_v8"
