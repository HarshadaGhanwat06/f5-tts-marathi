#!/usr/bin/env bash
#
# deploy_cartesia_v5.sh
#
# Deploy the pronunciation-focused model (Cartesia_Rasa_Combined_v5) to the
# live services: FastAPI on port 8000 and Gradio UI on port 7861.
#
# New checkpoint:
#   /root/f5-tts-marathi/f5tts/lib/python3.12/ckpts/Cartesia_Rasa_Combined_v5/model_last.pt
# Vocabulary (unchanged, 138 tokens):
#   /root/f5-tts-marathi/f5tts/data/Rasa_Marathi_Emotion_Female/vocab_extended.txt
#
# v5 = v3 set (2663) + prono vowel (528) fine-tuned with a ॅ/ॉ/ँ/ॉं
# pronunciation-focus, warm-started from the v3-trained checkpoint.
#
# Steps:
#   1. Backup production files (synthesize.py, api.py).
#   2. Force emotion_layer/synthesize.py to the v5 checkpoint + extended vocab.
#   3. Force the model identifier in emotion_layer/api.py to v5.
#   4. Restart FastAPI (8000) + Gradio UI (7861).
#   5. Validate health, model-info, and synthesis incl. ॅ/ॉ/ँ/ॉं targets.
#   6. Print rollback procedure.
#
# Usage:
#   bash scripts/deploy_cartesia_v5.sh
#
set -euo pipefail
...
NEW_CKPT="/root/f5-tts-marathi/f5tts/lib/python3.12/ckpts/Cartesia_Rasa_Combined_v5/model_last.pt"
...
echo "============================================================"
echo " F5-TTS Marathi Deployment - Cartesia_Rasa_Combined_v5 (ॅ/ॉ/ँ/ॉं)"
echo "============================================================"
...
# ---------------------------------------------------------------------------
# 2 & 3. Force synthesize.py / api.py to v5 paths
# ---------------------------------------------------------------------------
echo ""
echo "[2] Updating checkpoint & vocabulary in synthesize.py..."

# Force checkpoint path to v5 regardless of what is hardcoded
perl -pi -e 's{/root/f5-tts-marathi/f5tts/lib/python3\.12/ckpts/(Rasa_Marathi_Emotion_Female|Cartesia_Rasa_Combined_3h|Cartesia_Rasa_Combined_v2|Cartesia_Rasa_Combined_v3|Cartesia_Rasa_Combined_v4)/model_last\.pt}{/root/f5-tts-marathi/f5tts/lib/python3.12/ckpts/Cartesia_Rasa_Combined_v5/model_last.pt}g' "$SYNTH"
...
perl -pi -e 's{(Rasa_Marathi_Emotion_Female|Cartesia_Rasa_Combined_3h|Cartesia_Rasa_Combined_v2|Cartesia_Rasa_Combined_v3|Cartesia_Rasa_Combined_v4)/model_last\.pt}{Cartesia_Rasa_Combined_v5/model_last.pt}g' "$API"
...
grep -n "Cartesia_Rasa_Combined_v5\|model_identifier" "$API" | head -10 || true
...
nohup "$PYTHON" -m uvicorn emotion_layer.api:app --host 0.0.0.0 --port $API_PORT \
    > "$LOG_DIR/api_deploy_v5.log" 2>&1 &
disown
NEW_API_PID=$!
echo "  Started new API process PID $NEW_API_PID (log: $LOG_DIR/api_deploy_v5.log)"
...
nohup "$PYTHON" emotion_layer/ui.py --port $UI_PORT \
    > "$LOG_DIR/ui_deploy_v5.log" 2>&1 &
disown
NEW_UI_PID=$!
echo "  Started new UI process PID $NEW_UI_PID (log: $LOG_DIR/ui_deploy_v5.log)"
...
grep -E "Loading|checkpoint|vocab|Error|Traceback|error" "$LOG_DIR/api_deploy_v5.log" | tail -30 || true
...
# ---------------------------------------------------------------------------
# 8. Synthesis validation (ॅ/ॉ/ँ/ॉं + emotion regression)
# ---------------------------------------------------------------------------
echo ""
...
for tc in "${TEST_CASES[@]}"; do
    i=$((i+1))
    emotion="${tc%%|*}"; text="${tc#*|}"
    out_file="$REPO_ROOT/validation_output/test_v5_${i}.wav"
...
' 2>/dev/null)
    if [[ -z "$base64str" || "$base64str" == "__PARSE_ERR__" ]]; then
        ...
        echo "    log:  $(tail -3 "$LOG_DIR/api_deploy_v5.log")"
        continue
    fi
...
echo "      cd $REPO_ROOT && nohup $PYTHON -m uvicorn emotion_layer.api:app --host 0.0.0.0 --port $API_PORT > $LOG_DIR/api_rollback_v5.log 2>&1 & disown"
echo "      kill \$(ss -ltnp | grep ':$UI_PORT ' | grep -oP 'pid=\\K[0-9]+' | head -1); sleep 3"
echo "      cd $REPO_ROOT && nohup $PYTHON emotion_layer/ui.py --port $UI_PORT > $LOG_DIR/ui_rollback_v5.log 2>&1 & disown"
echo "============================================================"
echo ""
echo "IMPORTANT: Confirm the API startup log loaded Cartesia_Rasa_Combined_v5"
