#!/usr/bin/env bash
#
# deploy_cartesia_v2.sh
#
# Deploy the newly fine-tuned Marathi female F5-TTS model trained (4 epochs)
# on the COMPLETE combined dataset (1630 Rasa ~3h + 1033 Cartesia ~88min).
#
# New checkpoint:
#   /root/f5-tts-marathi/f5tts/lib/python3.12/ckpts/Cartesia_Rasa_Combined_v2/model_last.pt
# Vocabulary (extended, 85 entries - MUST match the trained embedding):
...
#   2. Ensures emotion_layer/synthesize.py points at Cartesia_Rasa_Combined_v2
#      model_last.pt and vocab_extended.txt.
#   3. Updates the model identifier in emotion_layer/api.py.
#   4. Restarts FastAPI on port 8000 and Gradio UI on port 7861.
#   5. Validates health, model loading, and synthesis.
#   6. Prints rollback procedure.
#
# The prior checkpoints (Rasa_Marathi_Emotion_Female, Cartesia_Rasa_Combined_3h)
# are NOT deleted; both remain as rollback options.
#
# Usage:
#   bash scripts/deploy_cartesia_v2.sh
#
set -euo pipefail
...
NEW_CKPT="/root/f5-tts-marathi/f5tts/lib/python3.12/ckpts/Cartesia_Rasa_Combined_v2/model_last.pt"
...
echo "============================================================"
echo " F5-TTS Marathi Deployment - Cartesia_Rasa_Combined_v2"
echo "============================================================"
...
# ---------------------------------------------------------------------------
# 2 & 3. Point synthesize.py / api.py at the v2 checkpoint + extended vocab
# ---------------------------------------------------------------------------
echo ""
echo "[2] Updating checkpoint & vocabulary in synthesize.py..."

# Force the checkpoint path to the v2 checkpoint regardless of what is
# currently hardcoded (Rasa.../model_last.pt or Cartesia_Rasa_Combined_3h/...).
perl -pi -e 's{/root/f5-tts-marathi/f5tts/lib/python3\.12/ckpts/(Rasa_Marathi_Emotion_Female|Cartesia_Rasa_Combined_3h)/model_last\.pt}{/root/f5-tts-marathi/f5tts/lib/python3.12/ckpts/Cartesia_Rasa_Combined_v2/model_last.pt}g' "$SYNTH"

# Force the extended vocab in synthesize.py (85 entries).
...
perl -pi -e 's{(Rasa_Marathi_Emotion_Female|Cartesia_Rasa_Combined_3h)/model_last\.pt}{Cartesia_Rasa_Combined_v2/model_last.pt}g' "$API"

# Verify edits
echo ""
echo "[VERIFY] synthesize.py model/vocab lines:"
grep -nE "ckpt_path\s*=|vocab_path\s*=|model_path\s*=|\.pt\"" "$SYNTH" | head -20 || true
echo ""
echo "[VERIFY] api.py model identifier:"
grep -n "Cartesia_Rasa_Combined_v2\|model_identifier" "$API" | head -10 || true
...
nohup "$PYTHON" -m uvicorn emotion_layer.api:app --host 0.0.0.0 --port $API_PORT \
    > "$LOG_DIR/api_deploy_v2.log" 2>&1 &
disown
NEW_API_PID=$!
echo "  Started new API process PID $NEW_API_PID (log: $LOG_DIR/api_deploy_v2.log)"
...
nohup "$PYTHON" emotion_layer/ui.py --port $UI_PORT \
    > "$LOG_DIR/ui_deploy_v2.log" 2>&1 &
disown
NEW_UI_PID=$!
echo "  Started new UI process PID $NEW_UI_PID (log: $LOG_DIR/ui_deploy_v2.log)"

# ---------------------------------------------------------------------------
# 6 & 7. Wait for health + validate
# ---------------------------------------------------------------------------
echo ""
...
grep -E "Loading|checkpoint|vocab|Error|Traceback|error" "$LOG_DIR/api_deploy_v2.log" | tail -30 || true
...
# ---------------------------------------------------------------------------
# 8. Synthesis validation (emotion regression suite)
# ---------------------------------------------------------------------------
echo ""
...
TEST_CASES=(
    ...
    "NEUTRAL|thank you"
    "NEUTRAL|मला office मध्ये drawing खूप आवडलं."
...
for tc in "${TEST_CASES[@]}"; do
    i=$((i+1))
    emotion="${tc%%|*}"; text="${tc#*|}"
    out_file="$REPO_ROOT/validation_output/test_v2_${i}.wav"
...
if curl -s -m 5 -o /dev/null -w "%{http_code}" "http://127.0.0.1:$UI_PORT/" >/dev/null 2>&1; then
    ...
    echo "  UI not yet responding (may still be loading)."
fi
...
echo "      cd $REPO_ROOT && nohup $PYTHON -m uvicorn emotion_layer.api:app --host 0.0.0.0 --port $API_PORT > $LOG_DIR/api_rollback_v2.log 2>&1 & disown"
echo "      kill \$(ss -ltnp | grep ':$UI_PORT ' | grep -oP 'pid=\\K[0-9]+' | head -1); sleep 3"
echo "      cd $REPO_ROOT && nohup $PYTHON emotion_layer/ui.py --port $UI_PORT > $LOG_DIR/ui_rollback_v2.log 2>&1 & disown"
echo "============================================================"
echo ""
echo "IMPORTANT: Confirm the API startup log loaded Cartesia_Rasa_Combined_v2"
